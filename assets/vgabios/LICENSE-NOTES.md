...
- `vgabios.bin` is the standard Bochs VGABIOS image from upstream commit
...
- The checked-in binary is the `VGABIOS-lgpl-latest.bin` file stored in that
  exact upstream commit. Its SHA-256 is recorded in `vgabios.bin.sha256`.
- Rebuilding uses the unmodified vendored source and a fixed upstream commit
  date. The Linux/WSL builder requires dev86 (`bcc` and `as86`).
