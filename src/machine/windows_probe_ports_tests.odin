...
test_windows_known_absent_probe_ports :: proc(t: ^testing.T) {
	...
	for port in ports {
		testing.expect_value(t, bus_io_read(&m.bus, port, 1), u32(0xFF))
		bus_io_write(&m.bus, port, 1, 0x5A)
	}
	testing.expect_value(t, bus_io_read(&m.bus, 0xE22, 2), u32(0xFFFF))
	bus_io_write(&m.bus, 0xE22, 2, 0x00E0)
	testing.expect(t, !m.bus.frozen)
	testing.expect_value(t, m.bus.unclassified_count, u64(0))
	testing.expect_value(t, m.bus.passive_count, u64(36))
}
