...
ide_irq_routing_test_ack_slave :: proc(t: ^testing.T, m: ^Machine, expected: u8) {
	...
	pic_out(&m.platform.pic, 0xA0, 0x20)
	pic_out(&m.platform.pic, 0x20, 0x20)
...
ide_irq_routing_test_compatibility_channels_drive_irq14_and_irq15 :: proc(t: ^testing.T) {
	...
	testing.expect(t, m.platform.pic.direct_asserted & (u16(1) << 14) != 0)
	testing.expect(t, m.platform.pic.direct_asserted & (u16(1) << 15) != 0)
...
ide_irq_routing_test_iden_disable_does_not_pulse_legacy_irqs :: proc(t: ^testing.T) {
	...
	testing.expect(t, machine_sync_pci_devices(m))
	testing.expect(t, !pci_pirq_is_asserted(&m.pci, PCI_AMD756_IDE_PIRQ))
	testing.expect(t, m.platform.pic.direct_asserted & (u16(3) << 14) == 0)
	testing.expect(t, m.platform.pic.slave.irr & 0xC0 == 0)
}
...
ide_irq_routing_test_pending_native_irq_survives_iose_toggle_without_new_edge :: proc(
	...
	ide_irq_routing_test_signal_primary(m, true)
	testing.expect(t, pci_pirq_is_asserted(&m.pci, PCI_AMD756_IDE_PIRQ))
	testing.expect(t, m.platform.pic.source_asserted[10] & (u8(1) << u8(Pic_Irq_Source.Pci_Pirq)) != 0)
	...
	testing.expect(t, pci_pirq_is_asserted(&m.pci, PCI_AMD756_IDE_PIRQ))
	testing.expect(t, m.platform.pic.source_asserted[10] & (u8(1) << u8(Pic_Irq_Source.Pci_Pirq)) != 0)
	testing.expect(t, m.platform.pic.slave.irr & 0x04 == 0)
	testing.expect(t, !pic_has_pending(&m.platform.pic))
