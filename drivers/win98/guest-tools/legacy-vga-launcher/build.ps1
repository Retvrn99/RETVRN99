...
$output = Join-Path $outputRoot 'PIFRUN.EXE'
$arguments = @(
    ...
    '-nodefaultlibs', 'main.c', '-luser32', '-lkernel32', '-lgcc',
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'The legacy VGA PIF launcher build failed.' }
}
finally {
    $env:PATH = $savedPath
}
...
Write-Output ("PIFRUN.EXE bytes={0} sha256={1}" -f $item.Length, $hash)
