...
| Unchained 256-color Mode X scanout | IBM register model plus documented compatible programming | Partial | `src/vga/scanout.odin` and `src/vga/legacy_addressing.odin` | The public-port probe in `src/machine/legacy_vga_probe_tests.odin` now verifies the complete 320/360 by 200/240/350/400/480 matrix, register-derived geometry and pitch, plane selection, first and last pixels, and a non-stale return to mode 3. Its scalar aperture histogram retains every exit without drops and records all ten normalized geometries. The integrated source regression also proves exact 800x600 sentinel restoration through a missing-partial-baseline recovery with zero upload failures. The console-only host contract now samples scanout at 60 Hz after a two-second stable-mode warmup, resets and arms the bounded histogram at the exact performance baseline, stops it when the ten-second window completes or restarts, and requires its exit count to equal the measured aperture delta. It preserves a completed window across mode exit and joins its exact generations, aperture-exit delta, and content-progress rate into the guest report only after shutdown validation. The paired gate requires all ten geometries three times in Scalar and Auto, at least 55 presented samples per second in Auto, and at least a 10x exit reduction. The exact-PIF Scalar mode-2 r05 run supplied the first runtime attribution: 1,215,019 retained exits with zero drops in the exact ten-second 360x200 window, all from one counted-store pattern, while presenting 514 content advances at 51.398 Hz. The source fast path for that pattern is bounded, fail-closed, callback-preserving, and covered by scalar-equivalence tests, but has no accelerated guest result yet. The r05 test config was staged outside Quake's `ID1` search path, so the guest remained in the renderer until the 180-second bound and never began desktop restoration or shutdown; the harness path is corrected without changing the approved PIF or production behavior. This row returns to `Conformant` only when the exact Quake PIF matrix, Auto performance, desktop restoration, and clean-shutdown gates all pass |
...
the exact static desktop without an upload failure. The exact Quake enumeration,
PIF matrix, performance, desktop restoration, and shutdown runs remain acceptance
gates, not completed conformance evidence. The accepted repair must preserve VGA
planar semantics, restore a full generation of the Windows desktop, and complete
normal shutdown before this row is promoted.

The evidence harness no longer accepts its earlier `host-join-required`
placeholders as a completed report. It retains the raw guest TSV separately,
requires fixed pre-PIF and restored-desktop capture identities plus one stable
ten-second Mode X measurement, requires the histogram to cover exactly that
window, validates D5 through DC, and only then atomically writes the canonical
joined TSV. Scalar controls may retain the MMIO-storm
diagnostic; Auto runs may not. The matrix-wide paired verifier consumes exactly
60 summaries and fails closed on missing cases, dropped histogram exits, rates
below 55 Hz, or less than a 10x reduction.

The VBE routing question has been decided twice: first recorded as a limit, then
...
| Clone-specific undocumented VGA/SVGA quirks | In target | Was out of target while IBM VGA and the pinned Bochs contracts were the only normative sources. Reversed on 2026-08-02: the guests this milestone exists for are DOS-era and early Windows 9x titles, and the behaviour they depend on is exactly what the clones accumulated. DOSBox-X is now a normative reference beside IBM, and a quirk it implements is a work item rather than a limit |
