...
| Scanline-ordered register, palette, and aperture changes | IBM programming considerations | Missing | Deferred production path discards raster changes; the VM-side raster path exists and is tested but is switched off in production | Designed in ADR 0012 as a bounded scan-line journal on the descriptor that the host replays during expansion. Landing as vertical slices by effect, palette splits first |
