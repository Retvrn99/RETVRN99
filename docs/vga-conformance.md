...
| Scanline-ordered register, palette, and aperture changes | IBM programming considerations | Missing | `src/vga/raster_journal.odin` carries a bounded scan-line journal of typed deltas on the descriptor and host expansion replays it. Only the DAC entry kind is recorded; display start, byte and PEL pan, attribute palette, overscan, and aperture or bank deltas are still discarded, so most of this row is unreached | `raster_journal_test_palette_splits_replay_above_and_below_the_split` drives two splits through the public DAC ports at timestamps inside one frame and proves three colour bands from a single palette index. Siblings prove a frame without mid-frame writes carries an empty journal and expands byte-identically, and that overflow marks the frame truncated, counts it in `raster_journal_truncations`, and falls back to expansion from the final register state. The VM-side raster path stays in the tree as the reference and remains switched off in production |
