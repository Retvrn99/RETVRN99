...
| Attribute 00h-0Fh internal palette | IBM 2-90 to 2-91 | Partial | `src/vga/ports.odin` gates read and write behind the Palette Address Source, and `src/vga/raster_journal.odin` carries mid-frame changes with that bit | `raster_journal_test_attribute_palette_split_replays_below_the_split` changes an entry mid-frame through the public port and requires the rows below to resolve through it, and `raster_journal_test_palette_source_blanks_the_rows_it_is_held_off_for` requires the blank band the address-source dance produces. The 256-colour semantics under Attribute 10h bit 7 remain |
