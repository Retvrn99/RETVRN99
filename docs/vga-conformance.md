...
| Unchained 256-color Mode X scanout | IBM register model plus documented compatible programming | Partial | `src/vga/scanout.odin` and `src/vga/legacy_addressing.odin` | The public-port probe in `src/machine/legacy_vga_probe_tests.odin` reaches the complete 320/360 by 200/240/350/400/480 target matrix at the registers, but records stale 320x200 presentation after the first case and stale indexed presentation after mode 3. The retained exact-PIF diagnosis also records a trapped-aperture exit storm and repeated `upload-failed` presentation after returning to 800x600. This row returns to `Conformant` only when the strict ten-mode probe, exact Quake PIF matrix, desktop restoration, and clean-shutdown gates all pass |
...
public-port probe reaches the ten-geometry target matrix but exposes a cached
320x200 frame across later modes and a stale indexed frame after mode 3. The
exact Quake enumeration and PIF matrix remain acceptance gates, not completed
conformance evidence. The accepted repair must preserve VGA planar semantics,
restore a full generation of the Windows desktop, and complete normal shutdown
before this row is promoted.
